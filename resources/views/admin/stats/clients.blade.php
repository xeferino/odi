...
@section('title', 'Estadísticas de los clientes')
@section('styles')
...
<div class="page-wrapper">
    <div class="container-fluid">
        <div class="row">
            <div class="col-6">
                <select class="select2 form-control custom-select" style="height:36px;" id="client">
                    <option value="-1">Cliente</option>
                    @foreach($clients as $client)
                    <option value="{{ $client->user_id }}">{{ $client->company }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-6">
...
<script>
    ...
    $(document).ready(function () {
        ...
        table = $('#table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('admin.stats.clients.data') }}",
                data: (data) => {
            ...
            footerCallback: function (row, data, start, end, display) {
                ...
                $(api.column(1).footer()).html((total_price / 100).toLocaleString('es-MX', {
                    style: 'currency',
                    currency: 'MXN'
                }));
                $(api.column(2).footer()).html(total_items);
            }
        });
